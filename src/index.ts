...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'OpenWeather');
}
...
/**
 * Say WHICH place was not found, in the message.
 *
 * OpenWeather answers an unknown city with `{"cod":"404","message":"city not
 * found"}`, and this helper used to hand that body straight back as
 * `{error: 404, message: "<that raw JSON, stringified>"}`. Two things went
 * wrong with it, and the second is the expensive one:
 *
 *  - `error` was the NUMBER 404, so a caller reading the field got an integer
 *    where every other pack returns a sentence.
 *  - the message never named the place the caller asked for. The gateway's
 *    classifier decides `not_found` vs `error` by SHAPE — the caller's own
 *    argument echoed back in the message text (error-class.ts, fleet #711) —
 *    so every unknown city booked as `error`, the tier that means PIPEWORX
 *    BROKE.
 *
 * Measured 2026-09-23: `current_weather` showed 15 errors on 21 requests, a
 * 71% error rate, across three unrelated external identities — while
 * `internal:monitor` succeeded 8 of 8 on the same tool and London answers
 * normally on demand. The tool was working the whole time. Somebody asked for
 * places OpenWeather does not carry and we filed it as our outage.
 *
 * This is the same defect sciencebase had two days ago (`item not found` with
 * the id in a sibling field), which is why it is worth stating as a rule
 * rather than a one-off: **an upstream's error body is not our error message.**
 * Echo the caller's input, or the miss is indistinguishable from a fault.
 */
async function owmGet(
  path: string,
  ...
  if (!res.ok) {
    ...
    if (res.status === 404) {
      return {
        found: false,
        error: subject
          ? `No OpenWeather location matching "${subject}". Check the spelling, or add a country code ` +
            'like "Springfield,US" — OpenWeather matches on its own gazetteer and does not fuzzy-match. ' +
            'geocode() resolves a place name to coordinates if you would rather pass lat/lon.'
          : 'OpenWeather has no location matching that request.',
        ...(subject ? { requested: subject } : {}),
      };
    }
    return { error: `OpenWeather returned HTTP ${res.status}: ${text.slice(0, 200)}` };
  }
  return res.json();
}
...
async function currentWeather(args: Record<string, unknown>, apiKey: string) {
  ...
  // What the caller actually asked for, echoed into any not-found message —
  // see owmGet. A lat/lon pair is a legitimate subject too: "no location at
  // 89.9, 0" is a real answer and still needs to name the coordinates.
  const subject = args.city ? String(args.city) : `${String(args.lat)}, ${String(args.lon)}`;
  const data = (await owmGet('/data/2.5/weather', params, subject)) as Record<string, any>;
...
async function forecast(args: Record<string, unknown>, apiKey: string) {
  ...
  // 89.9, 0" is a real answer and still needs to name the coordinates.
  const subject = args.city ? String(args.city) : `${String(args.lat)}, ${String(args.lon)}`;
  const data = (await owmGet('/data/2.5/forecast', params, subject)) as Record<string, any>;
  if (data && (data as any).error !== undefined) return data;
...
async function airQuality(args: Record<string, unknown>, apiKey: string) {
  ...
  const data = (await owmGet('/data/2.5/air_pollution', params, `${String(args.lat)}, ${String(args.lon)}`)) as Record<string, any>;
  if (data && (data as any).error !== undefined) return data;
...
async function geocode(args: Record<string, unknown>, apiKey: string) {
  ...
  const data = (await owmGet('/geo/1.0/direct', params, String(args.query ?? ''))) as any;
